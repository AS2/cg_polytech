...
HRESULT Plane::CompileShaderFromFile(const WCHAR* szFileName, LPCSTR szEntryPoint, LPCSTR szShaderModel, ID3DBlob** ppBlobOut)
{
  HRESULT hr = S_OK;
...
bool Plane::Frame(ID3D11DeviceContext* context, const std::vector<XMMATRIX>& worldMatricies, XMMATRIX viewMatrix, XMMATRIX projectionMatrix, XMFLOAT3 cameraPos) {
  ...
  // Count distances from cam to each plane and sort from furthest to nearest
  ...
  std::sort(dists_indxs.begin(), dists_indxs.end(), [](std::pair<float, UINT> a, std::pair<float, UINT> b)
    {
